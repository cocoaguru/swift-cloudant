...
class ViewPagingTests : XCTestCase {
    ...
    func testPageForward(){
        ...
        let viewPage = ViewPage(name: "paging", designDocumentID: "paging", databaseName: dbName, client: client!, pageSize: 5){ (page: [String : Any]?, token: ViewPage.Token?, error: Error?) -> ViewPage.Paging in
    ...
    func testPageForwardDescending(){
        ...
        let viewPage = ViewPage(name: "paging", designDocumentID: "paging", databaseName: dbName, client: client!, pageSize: 5, descending: true){ (page: [String : Any]?, token: ViewPage.Token?, error: Error?) -> ViewPage.Paging in
            XCTAssertNotNil(page)
            XCTAssertNil(error)
            if let page = page {
                if isFirst {
    ...
    func testPageForwardFinalPageLessThanPageSize(){
        ...
        let viewPage = ViewPage(name: "paging", designDocumentID: "paging", databaseName: dbName, client: client!, pageSize: 4){ (page: [String : Any]?, token: ViewPage.Token?, error: Error?) -> ViewPage.Paging in
            XCTAssertNotNil(page)
            XCTAssertNil(error)
    ...
    func testPageBackwardsUsingToken(){
        ...
        ViewPage.previous(token: token){ page, token, error in
    ...
    */
    ...
    func testPageForwardsUsingToken(){
        ...
        let tokenPageExpectation = self.expectation(description: "Paging views with token")
        ViewPage.next(token: token){ page, token, error in
    ...
    func testPageBackwardsUsingTokenString() throws {
        ...
        let stringToken = try token.seralised()
        
        
        let tokenPageExpectation = self.expectation(description: "Paging views with token")
        try ViewPage.previous(token: stringToken, client: self.client!){ page, token, error in
    ...
    func testPageForwardsUsingTokenString() throws {
        ...
        let tokenPageExpectation = self.expectation(description: "Paging views with token")
        let stringToken = try token.seralised()
        try ViewPage.next(token: stringToken, client: client!){ page, token, error in
